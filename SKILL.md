...
没有样例时，按文本类型定语气。博客、随笔、观点、个人化写作要保留作者的观点、不确定、矛盾心情、幽默和题外话，该有反应的地方可以加一句反应。参考、技术、法律、事实类文本保持中立、平实。公文、总结、新闻通稿里的套话（如“为……奠定基础”“具有重要意义”）是体裁要求的格式，按体裁保留，不当特征删。去掉特征只是一半工作；成品还得听起来像人写的。
...
**留意：** 让我们一起来看看、让我们来探讨、我们来拆解一下、你需要知道的是、接下来我们看、话不多说、温馨提示、说实话？、老实讲、不得不说、值得一提的是、众所周知、值得注意的是、需要指出的是、问题是、这么说吧，以及“有件事坑过我，所以注意了”这种随意版
...
**问题：** 中文 AI 少用被动，却爱用“进行/实现/得到 + 名词”代替直接动词，还爱省掉施动者。翻译腔还爱堆长定语链（“……的……的……”），把英文从句和 of 结构直译过来，读起来喘不过气；能拆短句就拆，能把定语挪后就挪。该用强动词就用强动词，该点名施动者就点名。但叙事和随笔里承前省主语、用话题链是正常的，别为点名施动者硬加主语；只有说明文里无主句让读者猜不到谁在做时，才补施动者。*单独算弱*。
**改前：**
> 无需配置文件。结果得到了自动的保留。
**改后：**
> 你不需要配置文件。系统会自动保留结果。
**改前（长定语链）：**
> 这是一个由三位来自北京的专家组成的委员会发布的关于城市更新报告的结论。
**改后：**
> 三位来自北京的专家组成委员会，发布了城市更新报告，这是它的结论。
...
**规则：** 终稿里的中文标点一律用全角（逗号、句号、顿号、冒号、分号、问号、叹号、引号、括号都用中文全角形态）。这条规则也约束你自己的输出：别一边把原文的半角改成全角，一边在终稿里又敲半角逗号。只有代码、行内代码、URL、纯英文夹杂，或目标格式明确要求半角时，才保留半角。数字、百分号、年份用半角，且数字与单位之间不加空格（写 50%、2024年，不写 50 %、2024 年）；全角规则只管标点，不管数字。
**问题：** 中文句子里混进半角标点、中英文标点混排、引号或括号内多余空格，是常见的 AI 痕迹。多数编辑器会自动把直引号转成弯引号，所以单看引号*单独算弱*；但中文句子里用半角逗号、冒号不是弱信号，该改。
**改前：**
